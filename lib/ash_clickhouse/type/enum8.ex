defmodule AshClickhouse.Type.ChEnum8 do
  @moduledoc """
  A ClickHouse-specific type for abstracting enums into a single type.

  This type is specifically designed to work with ClickHouse's Enum8 types and provides
  the same functionality as Ash.Type.Enum but with ClickHouse-specific optimizations.

  ClickHouse Enum8 requires key-value pairs where values are integers:
  - Enum8('option1' = 1, 'option2' = 2, 'option3' = 3)
  ...
  defmodule MyApp.TicketStatus do
    use AshClickhouse.Type.ChEnum8, values: [
  ...
  **IMPORTANT**: ClickHouse Enum8 requires integer values for each key.
  ...
  This type automatically creates the appropriate ClickHouse Enum8 type and handles:
  - Storage as ClickHouse Enum8 with proper key-value pairs
  ...
  defmacro __using__(opts) do
    quote location: :keep, generated: true, bind_quoted: [opts: opts] do
      ...
      @opts opts
      @values_map ChEnum8.build_values_map(opts[:values])
      @values ChEnum8.build_values_list(opts[:values])
      ...
      def ch_type(_constraints) do
        Ch.Types.enum8(Enum.map(@opts[:values], &normalize_value/1))
      end
    ...
    Enum.each(values, fn
      ...
      {value, details} when (is_atom(value) or is_binary(value)) and is_list(details) ->
        case Keyword.get(details, :value) do
          ...
          other ->
            raise "Enum8 requires integer values. Got #{inspect(other)} for #{inspect(value)}."
        end

      other ->
        raise(
          "`values` must be a list of {`atom | string`, integer} or {`atom | string`, [value: integer, description: string, label: string]} tuples for Enum8. Got #{inspect(other)}"
        )
    end)
