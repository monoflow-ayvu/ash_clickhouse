...
defmodule AshClickhouse.Mix.Helpers do
  @moduledoc false
    ...
    cond do
      ...
      opts[:domains] && opts[:domains] != "" ->
        ...
        resources =
          ...
          |> Enum.filter(&(Ash.DataLayer.data_layer(&1) == AshClickhouse.DataLayer))
          |> case do
            [] ->
              raise """
              No resources with `data_layer: AshClickhouse.DataLayer` found in the domains #{Enum.map_join(domains, ",", &inspect/1)}.

              Must be able to find at least one resource with `data_layer: AshClickhouse.DataLayer`.
              """
        ...
        |> Enum.flat_map(
          &[
            AshClickhouse.DataLayer.Info.repo(&1, :read),
            AshClickhouse.DataLayer.Info.repo(&1, :mutate)
          ]
        )
        |> Enum.uniq()
        |> case do
          [] ->
            ...
            The following resources were found with `data_layer: AshClickhouse.DataLayer`:
      ...
      true ->
        ...
        |> Enum.filter(fn repo ->
          Spark.implements_behaviour?(repo, AshClickhouse.Repo)
        end)
    end
  end
